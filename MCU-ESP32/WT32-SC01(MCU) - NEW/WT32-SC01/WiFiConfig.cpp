...
String serverIndex;
void set_servet_index(String Version)
{
    serverIndex =
   "<script src='https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js'></script>"
...
void WiFiConfig::WIFI_Connenct()
{
    ...
    while(true)
    {
      ...
      WiFi.config(ipAdress ,gateway ,subnet ,dns1 ,dns2 );
      
      String CountryStr = "JP";
      wifi_country_t mycountry;
      CountryStr.toCharArray(mycountry.cc , 2);
      mycountry.schan = 1;
      mycountry.nchan = 14;
      mycountry.policy = WIFI_COUNTRY_POLICY_MANUAL;
      ...
      tft->drawString(message , 0, ConnectWIFI_posy,FONT2);
      int retry = 0;
      while (WiFi.status() != WL_CONNECTED) //等待网络连接成功
      {
        if( retry >= 5)
        {
           ...
           tft->drawString("WIFI Connect timeout!" , 0, ConnectWIFI_posy,FONT2);
           ESP.restart();
           break;
        }
      ...
      if( this -> IsConnected)
      {
        ConnectWIFI_posy+= Text_Height;
        tft->setTextColor(TFT_BLACK , TFT_RED); 
        tft->drawString("WIFI Connected!" , 0, ConnectWIFI_posy,FONT2);
        break;
      }
      else
      {
