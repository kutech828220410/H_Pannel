...
void sub_RFID_program()
{
    ...
    {       
       if(flag_udp_232back)Serial.printf("RFID_Enable : %d\n" , rFID_Enable);
...
//            if(flag_udp_232back)Serial.printf("value: %d , RFID_len : %d \n" ,read_temp,RFID_len);
            RFID_len++;
            MyTimer_RFID.TickStop();
            MyTimer_RFID.StartTickTime(100);
     ...
     {
        if(flag_udp_232back)Serial.printf("station : %d , RFID_len : %d" ,station ,RFID_len);
...
bool Set_Beep(byte station)
{
   ...
   while(true)
   {
     ...
     if(flag_rx_ok == 8)
     {
        if(flag_udp_232back)Serial.printf("RFID Set Beep sucess , station : %d \n",station);
        return true;
     }
     else
     {        
   ...
   if(flag_udp_232back)Serial.printf("RFID Set Beep failed , station : %d \n",station);
   return false;    
}
