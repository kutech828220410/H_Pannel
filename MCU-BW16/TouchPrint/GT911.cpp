...
bool GT911::begin(int8_t intPin, int8_t rstPin, uint8_t addr, uint32_t clk) {
  ...
  if (_rstPin > 0) {
    Serial.print("GT911::begin _rstPin!!"); Serial.println("");
    delay(300);
    reset();
    delay(200);
  }
  ...
  if (intPin > 0) {
    Serial.print("GT911::begin _intPin!!"); Serial.println("");
    pinMode(_intPin, INPUT);
    attachInterrupt(_intPin, _gt911_irq_handler, FALLING);
  }
  Serial.print("GT911::begin begin!!"); Serial.println("");
  _wire->begin();
  Serial.print("GT911::begin setClock!!"); Serial.println("");
  Serial.print("GT911::begin beginTransmission!! addr:");Serial.print(_addr); Serial.println("");
  _wire->beginTransmission(_addr);
  if (_wire->endTransmission() == 0) {
    readInfo();
    Serial.print("GT911::begin sucessed!!"); Serial.println("");
    return true;
  }
  Serial.print("GT911::begin failed!!"); Serial.println("");
  return false;
}
void GT911::reset() {
  delay(1);
...
uint8_t GT911::read(uint16_t reg) {
  ...
  while (_wire->available()) {
    uint8_t data_temp = _wire->read();
    Serial.print("value : ");Serial.print(data_temp);Serial.println("");
    return data_temp;
  }
  Serial.print("value : ");Serial.print("failed!");Serial.println("");
  return 0;
}
...
bool GT911::readBytes(uint16_t reg, uint8_t *data, uint16_t size) {
  ...
  for (uint8_t i=0; i<size; i++) {
    data[i] = Wire.read();
    Serial.print("value : ");Serial.print(data[i]);Serial.println("");
  }
