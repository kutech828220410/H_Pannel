...
/**
  ...
  */
void *My_Psram_heap_allocmem(int size)
{
  ...
  if(!My_g_Psram_heap_inited)  My_Psram_heap_init();
...
/**
  ...
  * @retval none
  */
void My_Psram_reserved_heap_freemem(void *mem, int size)
{
  PsramMemChunk *prev;
  struct Psram_Heap* h = &g_Psram_heap;
  _irqL   irqL;
...
/**
  ...
  */
int My_Psram_reserve_free_size(void)
{
  int free_mem = 0;
  struct Psram_Heap* h = &g_Psram_heap;
  _irqL   irqL;
...
/**
  ...
  * @retval: header address of the memory allocated.
  */
void *My_Psram_reserve_malloc(int size)
{
  int *mem;

  size += sizeof(int);
  if ((mem = (int*)My_Psram_heap_allocmem(size))){
    *mem++ = size;
  }
...
/**
  ...
  * @retval: header address of the memory allocated.
  */
void *My_Psram_reserve_calloc(int num, int size)
{
  void *mem;

  if ((mem = My_Psram_reserve_malloc(num * size)))
...
/**
  ...
  * @retval none
  */
void My_Psram_reserve_free(void *mem)
{
  int *_mem = (int *)mem;
  ...
  {
    --_mem;
    My_Psram_reserved_heap_freemem(_mem, *_mem);
  }
}
