...
namespace H_Pannel_lib
{
    ...
    public class Communication
    {
        ...
        {
            using (Bitmap bitmap = TextToBitmap(Text, font, 1, width, height, Color.White, Color.Black, 0 , 0, Color.Black, horizontalAlignment))
            {
                if (bitmap == null) return true;
                int len = 0;
                int last_len = 0;
        ...
        static public bool EPD_360E_DrawImage(UDP_Class uDP_Class, string IP, Bitmap bmp)
        {
            if (!Basic.Net.Ping(IP, 2, 150))
            {
                Console.WriteLine($"EPD360E DrawImage start {DateTime.Now.ToDateTimeString()} : Ping Failed {IP} ");
                return false;
            }
            ...
            using (Bitmap inputBmp = ScaleImage(bmp, 600, 400))
            {
                using (Bitmap _bmp = DitheringProcessor.ApplyFloydSteinbergDithering(inputBmp, DitheringProcessor.DitheringMode.SixColor))
                {
                    ...
                    //EPD_6Color_BytesToBitmap(bytes, width, height).Save($@"{Environment.GetFolderPath(Environment.SpecialFolder.Desktop)}\EPD360E_BytesToBitmap.bmp");

                    MyTimer myTimer = new MyTimer();
                    myTimer.StartTickTime(50000);
                    flag_OK = EPD_DrawImageSPI_EPD360E(uDP_Class, IP, bytes, ((width / 2) * height) / EPD583_frameDIV);
                    if (ConsoleWrite) Console.WriteLine($"{IP}:{uDP_Class.Port} : EPD 360E DrawImage {string.Format(flag_OK ? "sucess" : "failed")}!   Time : {myTimer.GetTickTime().ToString("0.000")} ms");
                    return flag_OK;
        ...
        static public bool EPD_730F_DrawImage(UDP_Class uDP_Class, string IP, Bitmap bmp)
        {
            ...
            if (ConsoleWrite) Console.WriteLine($"{IP}:{uDP_Class.Port} : EPD 730F DrawImage {string.Format(flag_OK ? "sucess" : "failed")}!   Time : {myTimer.GetTickTime().ToString("0.000")} ms");
            return flag_OK;
        }
        static public bool EPD_730E_DrawImage(UDP_Class uDP_Class, string IP, Bitmap bmp)
        {
            if (!Basic.Net.Ping(IP, 2, 150))
            {
                Console.WriteLine($"EPD730E DrawImage start {DateTime.Now.ToDateTimeString()} : Ping Failed {IP} ");
                return false;
            }
            int EPD583_frameDIV = 0;
            ...
            using (Bitmap inputBmp = ScaleImage(bmp, 800, 480))
            {
                using (Bitmap _bmp = DitheringProcessor.ApplyFloydSteinbergDithering(inputBmp, DitheringProcessor.DitheringMode.SixColor))
                {
                    int width = _bmp.Width;
                    ...
                    BitmapToByte(_bmp, ref bytes, EPD_Type.EPD730E);

                    //EPD_6Color_BytesToBitmap(bytes, width, height).Save($@"{Environment.GetFolderPath(Environment.SpecialFolder.Desktop)}\EPD730E_BytesToBitmap.bmp");

                    MyTimer myTimer = new MyTimer();
                    myTimer.StartTickTime(50000);
                    flag_OK = EPD_DrawImageSPI_EPD730E(uDP_Class, IP, bytes, ((width / 2) * height) / EPD583_frameDIV);
                    if (ConsoleWrite) Console.WriteLine($"{IP}:{uDP_Class.Port} : EPD 730E DrawImage {string.Format(flag_OK ? "sucess" : "failed")}!   Time : {myTimer.GetTickTime().ToString("0.000")} ms");
                    return flag_OK;
        ...
        static public bool EPD_DrawImageSPI_EPD360E(UDP_Class uDP_Class, string IP, byte[] datas, int Split_DataSize)
        {
            int Width_Size = Split_DataSize;
            int NumOfArray = datas.Length / (Split_DataSize);
        ...
        static public bool EPD_DrawImageSPI_EPD730E(UDP_Class uDP_Class, string IP, byte[] datas, int Split_DataSize)
        {
            int Width_Size = Split_DataSize;
            int NumOfArray = datas.Length / (Split_DataSize);
            List<byte[]> list_datas = new List<byte[]>();
        ...
        public enum EPDColors
        {
            ...
            EPD_7IN3E_BLACK = 0x0,   /// 000
            EPD_7IN3E_WHITE = 0x1,  /// 001
            EPD_7IN3E_YELLOW = 0x2,  /// 010
            EPD_7IN3E_RED = 0x3,  /// 011
            ...
            EPD_2IN13B_BLACK = 0x0,
            EPD_2IN13B_WHITE = 0x1,
            EPD_2IN13B_YELLOW = 0x2,
            EPD_2IN13B_RED = 0x3,
        }
    
        static public Bitmap Storage_GetBitmap(Storage storage , double scale = 1)
        {
        ...
        public static unsafe void BitmapToByte(Bitmap bimage, ref byte[] _bytes, EPD_Type ePD_Type)
        {
            List<byte> list_bytes = new List<byte>();

            // 使用 Format24bppRgb 鎖定圖像
        ...
        private static unsafe void ConvertToEPD730F(byte* SrcPtr, int width, int height, int ByteOfWidth, List<byte> list_bytes)
        {
            int SrcWidthxY, SrcIndex;
            int[] R = new int[2], G = new int[2], B = new int[2];
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < (width / 2); x++)
                {
                    ...
                    B[0] = SrcPtr[SrcIndex]; G[0] = SrcPtr[SrcIndex + 1]; R[0] = SrcPtr[SrcIndex + 2];
                    B[1] = SrcPtr[SrcIndex + 3]; G[1] = SrcPtr[SrcIndex + 4]; R[1] = SrcPtr[SrcIndex + 5];
                    temp_bytes[0] = GetEPD730FColorByte(R[0], G[0], B[0]);
                    temp_bytes[1] = GetEPD730FColorByte(R[1], G[1], B[1]);
                    temp = (byte)(temp_bytes[0] | (temp_bytes[1] << 4));
                    list_bytes.Add(temp);
                }
            }
        }
        private static byte GetEPD730FColorByte(int r, int g, int b)
        {
            if (IsEqualColor(Color.White, r, g, b)) return (byte)EPDColors.EPD_7IN3F_WHITE;
            if (IsEqualColor(Color.Yellow, r, g, b)) return (byte)EPDColors.EPD_7IN3F_YELLOW;
            if (IsEqualColor(Color.Red, r, g, b)) return (byte)EPDColors.EPD_7IN3F_RED;
            if (IsEqualColor(Color.Green, r, g, b)) return (byte)EPDColors.EPD_7IN3F_GREEN;
            if (IsEqualColor(Color.Blue, r, g, b)) return (byte)EPDColors.EPD_7IN3F_BLUE;
            if (IsEqualColor(Color.Orange, r, g, b)) return (byte)EPDColors.EPD_7IN3F_ORANGE;
            if (IsEqualColor(Color.Black, r, g, b)) return (byte)EPDColors.EPD_7IN3F_BLACK;
            return 0;
        }

        private static readonly Color[] Palette_360E_6Colors = new Color[]
       {
            Color.Black,                             // index 0
            Color.White,                             // index 1
            Color.FromArgb(255, 243, 56),            // Yellow (index 2)
            Color.FromArgb(191, 0, 0),               // Red (index 3)
            Color.FromArgb(100, 64, 255),            // Blue (index 4)
            Color.FromArgb(67, 138, 28),             // Green (index 5)
       };
        private static byte GetNearestColorIndex_360E(int r, int g, int b)
        {
            int minDiff = int.MaxValue;
            int bestIndex = 0;

            for (int i = 0; i < Palette_360E_6Colors.Length; i++)
            {
                Color c = Palette_360E_6Colors[i];
                int dr = r - c.R;
                int dg = g - c.G;
                int db = b - c.B;
        ...
        private static unsafe void ConvertToEPD360E(byte* SrcPtr, int width, int height, int ByteOfWidth, List<byte> list_bytes)
        {
            int SrcWidthxY, SrcIndex;
            int[] R = new int[2], G = new int[2], B = new int[2];
            byte[] temp_bytes = new byte[2];
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < (width / 2); x++)
                {
                    ...
                    // 兩個像素
                    B[0] = SrcPtr[SrcIndex + 0];
                    G[0] = SrcPtr[SrcIndex + 1];
                    R[0] = SrcPtr[SrcIndex + 2];

                    B[1] = SrcPtr[SrcIndex + 3];
                    G[1] = SrcPtr[SrcIndex + 4];
                    R[1] = SrcPtr[SrcIndex + 5];

                    temp_bytes[0] = GetNearestColorIndex_360E(R[0], G[0], B[0]);
                    temp_bytes[1] = GetNearestColorIndex_360E(R[1], G[1], B[1]);

                    temp = (byte)(temp_bytes[1] | (temp_bytes[0] << 4));
                    list_bytes.Add(temp);
                }
            }
        }

        private static readonly Color[] Palette_730E_6Colors = new Color[]
        {
            Color.Black,                             // index 0
            Color.White,                             // index 1
            Color.FromArgb(255, 243, 56),            // Yellow (index 2)
        ...
        private static byte GetNearestColorIndex_730E(int r, int g, int b)
        {
            int minDiff = int.MaxValue;
            int bestIndex = 0;

            for (int i = 0; i < Palette_730E_6Colors.Length; i++)
            {
                Color c = Palette_730E_6Colors[i];
                int dr = r - c.R;
                int dg = g - c.G;
                int db = b - c.B;
                int diff = dr * dr + dg * dg + db * db;
        ...
        private static unsafe void ConvertToEPD730E(byte* SrcPtr, int width, int height, int ByteOfWidth, List<byte> list_bytes)
        {
            int SrcWidthxY, SrcIndex;
            int[] R = new int[2], G = new int[2], B = new int[2];
            byte[] temp_bytes = new byte[2];
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < (width / 2); x++)
                {
                    ...
                    temp_bytes[0] = GetNearestColorIndex_730E(R[0], G[0], B[0]);
                    temp_bytes[1] = GetNearestColorIndex_730E(R[1], G[1], B[1]);
        ...
        public static Bitmap EPD_6Color_BytesToBitmap(byte[] data, int width, int height)
        {
            ...
            Dictionary<byte, Color> colorMap = new Dictionary<byte, Color>()
            {
                [0x0] = Color.Black,
                [0x1] = Color.White,
                [0x2] = Color.FromArgb(255, 243, 56),     // Yellow
                [0x3] = Color.FromArgb(191, 0, 0),         // Red
                [0x5] = Color.FromArgb(100, 64, 255),      // Blue
                [0x6] = Color.FromArgb(67, 138, 28)        // Green
            };
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x += 2)
                {
                    ...
                    byte color1 = (byte)(b & 0x0F);
                    byte color2 = (byte)((b >> 4) & 0x0F);

                    bmp.SetPixel(x, y, colorMap.ContainsKey(color1) ? colorMap[color1] : Color.Magenta);
                    bmp.SetPixel(x + 1, y, colorMap.ContainsKey(color2) ? colorMap[color2] : Color.Magenta);
                }
            }
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < (width / 4); x++)
                {
                    ...
                    for (int m = 0; m < 4; m++)
                    {
                        ...
                        temp_bytes[m] = GetEPD579GColorByte(R[m], G[m], B[m]);
                    }
        ...
        private static byte GetEPD579GColorByte(int r, int g, int b)
        {
            if (IsEqualColor(Color.White, r, g, b)) return (byte)EPDColors.EPD_5IN79G_WHITE;
            if (IsEqualColor(Color.Black, r, g, b)) return (byte)EPDColors.EPD_5IN79G_BLACK;
            if (IsEqualColor(Color.Red, r, g, b)) return (byte)EPDColors.EPD_5IN79G_RED;
            if (IsEqualColor(Color.Yellow, r, g, b)) return (byte)EPDColors.EPD_5IN79G_YELLOW;
            return 0;
        }

        private static unsafe void ConvertToEPD213_BRW(byte* SrcPtr, int width, int height, int ByteOfWidth, List<byte> list_bytes)
        {
            int SrcWidthxY, SrcIndex;
            int[] R = new int[4], G = new int[4], B = new int[4];
            byte[] temp_bytes = new byte[4];
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < (width / 4); x++)
                {
                    ...
                    for (int m = 0; m < 4; m++)
                    {
                        ...
                        G[m] = SrcPtr[SrcIndex + m * 3 + 1];
                        R[m] = SrcPtr[SrcIndex + m * 3 + 2];
                        temp_bytes[m] = GetEPD213ColorByte(R[m], G[m], B[m]);
                    }
                    temp = (byte)((temp_bytes[0] << 6) | (temp_bytes[1] << 4) | (temp_bytes[2] << 2) | temp_bytes[3]);
                    list_bytes.Add(temp);
                }
        ...
        private static byte GetEPD213ColorByte(int r, int g, int b)
        {
            if (IsEqualColor(Color.White, r, g, b)) return (byte)EPDColors.EPD_2IN13B_WHITE;
            if (IsEqualColor(Color.Black, r, g, b)) return (byte)EPDColors.EPD_2IN13B_BLACK;
            if (IsEqualColor(Color.Red, r, g, b)) return (byte)EPDColors.EPD_2IN13B_RED;
            return 0;
        }
        ...
        private static int GetBitmapSkip(int width, int bytesPerPixel)
        {
            int stride = ((width * bytesPerPixel + 3) / 4) * 4;
            return stride - (width * bytesPerPixel);
        }
        ...
        {
            Size string_size = TextRenderer.MeasureText(text, font);
            return TextToBitmap(text, font, 1, string_size.Width, string_size.Height, Color.Black, Color.White, 0 , 0, Color.Black, HorizontalAlignment.Left);
        }
        ...
        {
            if (SrcBitmap == null || dstWidth <= 0 || dstHeight <= 0) return null;
            ...
            using (Graphics g = Graphics.FromImage(DstBitmap))
            {
                ...
                // 計算比例與置中位置
                float scaleX = (float)dstWidth / SrcBitmap.Width;
                float scaleY = (float)dstHeight / SrcBitmap.Height;
                float scale = Math.Min(scaleX, scaleY);

                int scaledWidth = (int)(SrcBitmap.Width * scale);
                int scaledHeight = (int)(SrcBitmap.Height * scale);
                int offsetX = (dstWidth - scaledWidth) / 2;
                int offsetY = (dstHeight - scaledHeight) / 2;

                g.DrawImage(SrcBitmap, new Rectangle(offsetX, offsetY, scaledWidth, scaledHeight),
                    0, 0, SrcBitmap.Width, SrcBitmap.Height, GraphicsUnit.Pixel);
            }
            return DstBitmap;
        }
