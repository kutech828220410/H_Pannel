...
namespace H_Pannel_lib
{
    public class DrawerUI_EPD_1020 : DrawerUI
    {
        ...
        {
            if (uDP_Class == null) return false;
            return Communication.EPD_1020_DrawImage(uDP_Class, IP, bmp);
        }
        ...
        public DrawerUI_EPD_1020()
        {
            this.TableName = "EPD1020_Jsonstring";
            WS2812_Init();

            this.DeviceTableMouseDownRightEvent += DrawerUI_EPD_1020_DeviceTableMouseDownRightEvent;
            this.UDP_DataReceiveMouseDownRightEvent += DrawerUI_EPD_1020_UDP_DataReceiveMouseDownRightEvent;
        ...
        {
            int width = DrawerUI_EPD_1020.Pannel_Width;
            int height = DrawerUI_EPD_1020.Pannel_Height;
            Bitmap bitmap = new Bitmap(DrawerUI_EPD_1020.Pannel_Width, DrawerUI_EPD_1020.Pannel_Height);
            Graphics g = Graphics.FromImage(bitmap);
                ...
                if (dialog_ContextMenuStrip.ShowDialog() == DialogResult.Yes)
                {
                    ...
                    else if (dialog_ContextMenuStrip.Value == ContextMenuStrip_UDP_DataReceive_參數設定.設為有鎖控.GetEnumName())
                    {
                        ...
                        for (int i = 0; i < iPEndPoints.Count; i++)
                        {
                            ...
                            Drawer drawer = this.SQL_GetDrawer(IP);
                            if (drawer == null) continue;
                            drawer.SetDeviceType(DeviceType.EPD1020_lock);
                            taskList.Add(Task.Run(() =>
                            {
                                SQL_ReplaceDrawer(drawer);
                            }));
                    ...
                    else if (dialog_ContextMenuStrip.Value == ContextMenuStrip_UDP_DataReceive_參數設定.設為無鎖控.GetEnumName())
                    {
                        ...
                        for (int i = 0; i < iPEndPoints.Count; i++)
                        {
                            ...
                            Drawer drawer = this.SQL_GetDrawer(IP);
                            if (drawer == null) continue;
                            drawer.SetDeviceType(DeviceType.EPD1020);
                            taskList.Add(Task.Run(() =>
                            {
                                SQL_ReplaceDrawer(drawer);
                            }));
        ...
        private void DrawerUI_EPD_1020_DeviceTableMouseDownRightEvent(string selectedText, List<System.Net.IPEndPoint> iPEndPoints)
        {
            if (selectedText == ContextMenuStrip_Main.畫面設置.GetEnumName())
            {
