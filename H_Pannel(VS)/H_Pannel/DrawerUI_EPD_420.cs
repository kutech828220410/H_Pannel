...
{
    public class DrawerUI_EPD_420 : DrawerUI
    {
        [Serializable]
        ...
        static public Bitmap Get_Drawer_Barcode_bmp(Drawer drawer)
        {
            ...
            if (ip_array.Length == 4)
            {
                ...
                g.DrawString(ip, new Font("微軟正黑體", 8, FontStyle.Bold), new SolidBrush(Color.Black), (DrawerUI_EPD_420.Pannel_Width - size_IP.Width), (DrawerUI_EPD_420.Pannel_Height - size_IP.Height));
            }
            g.Dispose();
            return bitmap;
        }
        ...
        public DrawerUI_EPD_420()
        {
            this.TableName = "EPD420_Jsonstring";
            WS2812_Init();

            this.DeviceTableMouseDownRightEvent += DrawerUI_EPD_420_DeviceTableMouseDownRightEvent;
            this.UDP_DataReceiveMouseDownRightEvent += DrawerUI_EPD_420_UDP_DataReceiveMouseDownRightEvent;
        ...
        private void SqL_DataGridView_DeviceTable_DataGridRefreshEvent()
        {
            for (int i = 0; i < this.sqL_DataGridView_DeviceTable.dataGridView.Rows.Count; i++)
            {
                ...
                if(drawer != null)
                {
                    ...
                    if(drawer.Boxes[0][0].DeviceType == DeviceType.EPD420_lock)
                    {
        ...
        {
            int width = DrawerUI_EPD_420.Pannel_Width;
            int height = DrawerUI_EPD_420.Pannel_Height;
            Bitmap bitmap = new Bitmap(DrawerUI_EPD_420.Pannel_Width, DrawerUI_EPD_420.Pannel_Height);
            Graphics g = Graphics.FromImage(bitmap);
        ...
        private void DrawerUI_EPD_420_UDP_DataReceiveMouseDownRightEvent(string selectedText, List<System.Net.IPEndPoint> iPEndPoints)
        {
                ...
                if (dialog_ContextMenuStrip.ShowDialog() == DialogResult.Yes)
                {
                    ...
                    else if (dialog_ContextMenuStrip.Value == ContextMenuStrip_UDP_DataReceive_參數設定.設為有鎖控.GetEnumName())
                    {
                        ...
                        for (int i = 0; i < iPEndPoints.Count; i++)
                        {
                            ...
                            Drawer drawer = this.SQL_GetDrawer(IP);
                            if (drawer == null) continue;
                            drawer.SetDeviceType(DeviceType.EPD420_lock);
                            taskList.Add(Task.Run(() =>
                            {
                                SQL_ReplaceDrawer(drawer);
                            }));
                    ...
                    else if (dialog_ContextMenuStrip.Value == ContextMenuStrip_UDP_DataReceive_參數設定.設為無鎖控.GetEnumName())
                    {
                        ...
                        for (int i = 0; i < iPEndPoints.Count; i++)
                        {
                            ...
                            Drawer drawer = this.SQL_GetDrawer(IP);
                            if (drawer == null) continue;
                            drawer.SetDeviceType(DeviceType.EPD420);
                            taskList.Add(Task.Run(() =>
                            {
                                SQL_ReplaceDrawer(drawer);
                            }));
        ...
        private void DrawerUI_EPD_420_DeviceTableMouseDownRightEvent(string selectedText, List<System.Net.IPEndPoint> iPEndPoints)
        {
            if (selectedText == ContextMenuStrip_Main.畫面設置.GetEnumName())
            {
