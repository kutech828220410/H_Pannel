...
{
    public partial class EPD_420_Pannel : UserControl
    {
        ...
        public EPD_420_Pannel()
        {
            InitializeComponent();
        }

        private void EPD_420_Pannel_Load(object sender, EventArgs e)
        {
        ...
        public void BoxLightOn(Color color)
        {
            ...
            {
                CurrentDrawer.LED_Bytes = DrawerUI_EPD_420.Get_Drawer_LEDBytes_UDP(uDP_Class, CurrentDrawer.IP);
                DrawerUI_EPD_420.Set_Drawer_LED_UDP(uDP_Class, CurrentDrawer, cols, rows, color);
            }));
        ...
        public void BoxLightOff()
        {
            ...
            {
                CurrentDrawer.LED_Bytes = DrawerUI_EPD_420.Get_Drawer_LEDBytes_UDP(uDP_Class, CurrentDrawer.IP);
                DrawerUI_EPD_420.Set_Drawer_LED_UDP(uDP_Class, CurrentDrawer, cols, rows, Color.Black);
            }));
            Task allTask = Task.WhenAll(taskList);
        }
        public void AllLightOff()
        {
            ...
            taskList.Add(Task.Run(() =>
            {
                DrawerUI_EPD_420.Set_LED_Clear_UDP(uDP_Class, CurrentDrawer);
            }));
            Task allTask = Task.WhenAll(taskList);
        }
        public void AllLightOn(Color color)
        {
            ...
            taskList.Add(Task.Run(() =>
            {
                DrawerUI_EPD_420.Set_LED_UDP(uDP_Class, CurrentDrawer, color);
            }));
            Task allTask = Task.WhenAll(taskList);
        }
        public void PannelLightOn(Color color)
        {
            ...
            {
                CurrentDrawer.LED_Bytes = DrawerUI_EPD_420.Get_Drawer_LEDBytes_UDP(uDP_Class, CurrentDrawer.IP);
                DrawerUI_EPD_420.Set_Pannel_LED_UDP(uDP_Class, CurrentDrawer, color);
            }));
            Task allTask = Task.WhenAll(taskList);
        }
        public void PannelLightOff()
        {
            ...
            {
                CurrentDrawer.LED_Bytes = DrawerUI_EPD_420.Get_Drawer_LEDBytes_UDP(uDP_Class, CurrentDrawer.IP);
                DrawerUI_EPD_420.Set_Pannel_LED_UDP(uDP_Class, CurrentDrawer, Color.Black);
            }));
            Task allTask = Task.WhenAll(taskList);
        }
        virtual public Bitmap Get_Drawer_bmp(Drawer drawer)
        {
            return DrawerUI_EPD_420.Get_Drawer_bmp(drawer);
        }
        virtual public Bitmap Get_Drawer_Barcode_bmp(Drawer drawer)
        {
            return DrawerUI_EPD_420.Get_Drawer_Barcode_bmp(drawer);
        }
        ...
        {
            return DrawerUI_EPD_420.Get_Box_Combine(currentDrawer, box);
        }
            ...
            else if(e.Button == MouseButtons.Right)
            {
                ...
                if (dialog_ContextMenuStrip.ShowDialog() == DialogResult.Yes)
                {
                    ...
                    else if (dialog_ContextMenuStrip.Value == ContextMenuStrip_Main.上傳資料.GetEnumName())
                    {
                        ...
                        taskList.Add(Task.Run(() =>
                        {
                            DrawerUI_EPD_420.DrawToEpd_UDP(uDP_Class, CurrentDrawer);
